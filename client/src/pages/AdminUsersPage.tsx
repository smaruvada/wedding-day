...
const hostTypes = ["bride", "maid_of_honor", "planner", "other"];
...
function UserForm({ user, onClose }: { user?: AdminUser; onClose: () => void }) {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    mutation.mutate({
      ...
      role,
      hostType: role === "host" ? form.get("hostType") : null,
    });
  };
  return <form onSubmit={submit}><Stack>
    ...
    <TextInput required={!user} name="password" type="password" label={user ? "New password (optional)" : "Password"} minLength={8} />
...
export function AdminUsersPage() {
  ...
  return <Layout>
    ...
    <Table striped highlightOnHover withTableBorder>
      <Table.Thead><Table.Tr><Table.Th>ID</Table.Th><Table.Th>Name</Table.Th><Table.Th>Email / username</Table.Th><Table.Th>Role</Table.Th><Table.Th>Host type</Table.Th><Table.Th>Event</Table.Th><Table.Th>Created</Table.Th><Table.Th /></Table.Tr></Table.Thead>
      <Table.Tbody>{users.map((user) => <Table.Tr key={user.id}><Table.Td>{user.id}</Table.Td><Table.Td>{user.name}</Table.Td><Table.Td>{user.email}</Table.Td><Table.Td>{user.role}</Table.Td><Table.Td>{user.hostType ?? "—"}</Table.Td><Table.Td>{user.eventId}</Table.Td><Table.Td>{new Date(user.createdAt).toLocaleDateString()}</Table.Td><Table.Td><Button size="xs" variant="light" onClick={() => setEditing(user)}>Edit</Button></Table.Td></Table.Tr>)}</Table.Tbody>
    </Table>
